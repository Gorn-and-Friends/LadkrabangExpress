...
class Ticket{
    ...
    static async makeSeatLayout(req,res){
        ...
        try{
            ...
            for(let i=0 ; i < numberOfCoach ; i++){ //coach
                let temp = []
                for(let j = 0 ; j < seatRowPerCoach ; j++){ //
                    for(let k in columnTemp){
                        ...
                        for(let a = 0; a < reservedSeat.length; a++){
                            ...
                            if(reservedSeat[a].coach == i+1 && reservedSeat[a].row == j+1 && reservedSeat[a].column == columnTemp[k]){
                                isReserv = true
                                console.log(k)
                                break
                            }
    ...
    static async findReservedSeat(trainID,date){
        return await ticketModel.find({$and:[{"train_id": trainID},{"date": date}]})
    }
